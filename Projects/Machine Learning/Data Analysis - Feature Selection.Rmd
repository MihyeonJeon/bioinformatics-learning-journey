...
```{r}
# Change the target value to factor column
PPI_data$p_interface <- as.factor(PPI_data$p_interface)

# Remove some features (not treated as input + text)
PPI_data_wt = subset(PPI_data, select = -c(X,uniprot_id,aa_ProtPosition, domain, sequence, Rlength)) # wt = with target variable

# Remove the answer
PPI_data_nt = subset(PPI_data_wt, select = -c(p_interface)) # nt = no target variable
```
...
# oversampled_PPI <- oversample(train, method=c("SMOTE"), classAttr = "p_interface", ratio = 1)

oversampled_PPI <- SMOTE(p_interface~., train, perc.over = 630.407865, perc.under = 121.734644)

# library(DMwR)
# oversampled_PPI <- SMOTE(default ~ ., train, k = 5, perc.over = 100)
...
```{r with normalized data}
matrix_PPI <- as.matrix(normalized_PPI)
ssqtotal <- sum(matrix_PPI*matrix_PPI) # calculates the sum of squares
USV <- svd(matrix_PPI)
...
We can see the principal components don't explain the variance as much as expected.

2.  Original Data

```{r}
matrix_PPI <- as.matrix(PPI_data_nt)
ssqtotal <- sum(matrix_PPI*matrix_PPI) # calculates the sum of squares
USV <- svd(matrix_PPI)
T <- USV$u %*% diag(USV$d)          # Calculate scores
P <- USV$v                          # Loadings
...
```{r}
barplot(P[,1], main="Loadings of the first PC")
```
...
```{r}
# LDA_PPI_normalized <- subset(oversampled_PPI, select = -c(Rlength, p_interface))
# LDA_PPI_normalized <- subset(PPI_data_wt, select = -c(Rlength, p_interface))
# LDA_PPI_normalized <- scale(LDA_PPI_normalized)
# LDA_PPI_normalized <- data.frame(LDA_PPI_normalized)
# LDA_PPI_normalized$p_interface <- PPI_data_wt$p_interface
```

```{r}
norm_train <- data.frame(norm_train_x, norm_train_y)
norm_test <- data.frame(norm_test_x, norm_test_y)
```
...
# How well LDA model correctly predicted
# mean(predicted_lda_normalized$class == norm_test$p_interface)
```
...
```{r}
plot(model_lda_normalized$scaling, main = "Coefficients of LDA with normalized data")
```

```{r}
max(model_lda_normalized$scaling)
# names(train_normalized)[which(abs(model_lda_normalized$scaling)>=100)]
```

```{r}
# Summary of coefficients
summary(sort(abs(model_lda_normalized$scaling),decreasing = TRUE))
```

```{r}
# features with top 25% coefficients
names(train)[which(abs(model_lda_normalized$scaling)>=0.123947)]
```

2.  Original data

```{r LDA}
model_lda <- lda(p_interface~., data=downsample_PPI)
```

```{r}
predicted_lda <- predict(model_lda, test)
# How well LDA model correctly predicted
# mean(predicted_lda$class == test$p_interface)
```

```{r}
table(predicted_lda$class, test$p_interface)
```

```{r}
# Summary of coefficients
summary(sort(abs(model_lda$scaling),decreasing = TRUE))

```

```{r}
# features with top 25% coefficients
names(train_x)[which(abs(model_lda$scaling)>=0.91613)]
```

```{r}
# Make predictions on test set
pred <- predict(model_lda, newdata = test[,1:131])

# Create ROC curve
pred <- prediction(pred$posterior[,2], test[,132])
perf <- performance(pred, measure = "tpr", x.measure = "fpr")
plot(perf)
```

```{r}
confusionMatrix(predicted_lda$class, test_y)
```

```{r}
auc <- roc(test_y, as.numeric(predicted_lda$class))
auc$auc #
```
...
```{r}
set.seed(38)
rf_PPI = randomForest(p_interface ~ ., data=train, importance=TRUE)

yhat_rf = predict(rf_PPI, test, type="class")
confusionMatrix(yhat_rf, test$p_interface)


# varImpPlot(rf_PPI, scale=FALSE, n.var=10)
```

```{r}

pred=predict(rf_PPI,type = "prob")
perf = prediction(pred[,2], test$p_interface)
# 1. Area under curve
auc = performance(norm_perf, "auc")
auc
# 2. True Positive and Negative Rate
pred2 = performance(perf, "tpr","fpr")
# 3. Plot the ROC curve
plot(pred2,main="ROC Curve for Random Forest with Original data",col=2,lwd=2)
abline(a=0,b=1,lwd=2,lty=2,col="gray")
```

Except for one variable in Mean Decrease Accuracy, both models (normalized and original data) show same importance of features.
...
# set.seed(123)
# trainIndex <- createDataPartition(iris[, outcome], p = 0.8, list = FALSE)
...
```{r}
lasso_x <- as.matrix(train_x) 
lasso_y <- as.matrix(as.numeric(train_y)) 

# PPI_x <- as.matrix(dplyr::select(train, -p_interface)) 
# lgrid <- c(seq(0, 1, by=0.1), seq(2, 9, by=1), seq(10, 100, by=10)) 
# lasso_model <- glmnet(lasso_x, lasso_y, alpha=1, lambda=lgrid, standardize=FALSE, center = FALSE)

```

```{r}
# Print cross-validated mean squared error for different values of lambda 
cv_lasso <- cv.glmnet(lasso_x, lasso_y, alpha = 1,family="binomial") 
...
```{r}
# Features ~35
lasso_coef_df <- data.frame(c(colnames(train_x)), lasso_coef[2:132,])
colnames(lasso_coef_df) <- c('features','coefficient')

cat(lasso_coef_df[order(abs(lasso_coef_df$coefficient), decreasing = TRUE), ][1:35,1])
```
...
```{r}
# Predict
lasso_test_y <- as.matrix(as.numeric(test_y))
lasso_test_x <- as.matrix(test_x)

# predictions_train <- predict(cv, s = lamda_best, newx = lasso_x)
# eval_results(lasso_y, predictions_train, train)
lamda_best <- cv_lasso$lambda.min

predicted_lasso <- predict(cv_lasso, s = lamda_best, newx = lasso_test_x, type = "class")
predicted_lasso <- ifelse(predicted_lasso == 1, 0, 1)
# eval_results(lasso_test_y, predictions_test, test)


```

```{r}
table(predicted_lasso)
```

```{r}
# library(ROCR)
# pred <- prediction(cv_lasso, test_y)
# perf <- performance(pred, measure = "tpr", x.measure = "fpr")
# plot(perf)

# library(pROC)
auc <- roc(test_y, predicted_lasso)
auc$auc #
```
...
# lgrid <- c(seq(0, 1, by=0.1), seq(2, 9, by=1), seq(10, 100, by=10)) 
# norm_lasso_model <- glmnet(n_lasso_x, n_lasso_y, alpha=1, lambda=lgrid, standardize=FALSE, center = FALSE) 
...
# lasso_test_x <- as.matrix(dplyr::select(test, -p_interface)) 
...
cv_en <- cv.glmnet(lasso_x, lasso_y, alpha = 0.5, family='binomial')
...
# fit_final <- glmnet(lasso_x, lasso_y, alpha = 0.5, lambda = lambda_opt)

# Predict on the test set
predicted_en <- predict(cv_en, newx = lasso_test_x, s=lambda_opt, type = "class")
predicted_en <- ifelse(predicted_en == 1, 0, 1)
```
...
# library(pROC)
en_auc <- roc(test_y, predicted_en)
en_auc$auc #
```

```{r}
# Extract the coefficients of the final model
coef_final <- coef(cv_en)

# Identify the non-zero coefficients (i.e., important features)
important_features <- which(coef_final[-1,] != 0)

```

```{r}
# Features ~35
el_coeff <- data.frame(colnames(train_x)[important_features],coef_final[important_features])
colnames(el_coeff) <- c('features','coefficient')
cat(el_coeff[order(abs(el_coeff$coefficient), decreasing = TRUE), ][1:35,1])
```
...
# Fit the final elastic net model using the optimal lambda
# fit_final <- glmnet(n_lasso_x, n_lasso_y, alpha = 0.5, lambda = lambda_opt)
...
# model_lda <- train(Species ~ ., data = train, method = "lda", trControl = trainControl(method = "cv"))
# cv_lasso <- train(Species ~ ., data = train, method = "glmnet", trControl = trainControl(method = "cv"), tuneGrid = expand.grid(alpha = 1, lambda = seq(0.001, 0.1, length.out = 10)))
# cv_en <- train(Species ~ ., data = train, method = "glmnet", trControl = trainControl(method = "cv"), tuneGrid = expand.grid(alpha = 0.5, lambda = seq(0.001, 0.1, length.out = 10)))
